...
test_bench *new_ordinary_tb(int nearest_points) {
    ...
    this_->input_mask = random_byte(random_in(0, 8));
...
void generate_vhdl(FILE *fp, test_bench *tb, int testid) {
    ...
    fprintf(fp, "library vunit_lib;\n");
    fprintf(fp, "context vunit_lib.vunit_context;\n");
    
    fprintf(fp, "entity autogen_test_bench_%d is\n", testid);
    fprintf(fp, "    generic (runner_cfg : string);\n");
    ...
    fprintf(fp, "\t\t\t\t\tRAM(to_integer(unsigned(mem_address))) <= mem_i_data;\n");
    fprintf(fp, "\t\t\t\t\tmem_o_data                     <= mem_i_data after 2 ns;\n");
    fprintf(fp, "\t\t\t\telse\n");
    fprintf(fp, "\t\t\t\t\tmem_o_data <= RAM(to_integer(unsigned(mem_address))) after 2 ns;\n");
    fprintf(fp, "\t\t\t\tend if;\n");
    fprintf(fp, "\t\t\tend if;\n");
    fprintf(fp, "\t\tend if;\n");
    ...
    fprintf(fp, "test:\n\tprocess is\n");
    fprintf(fp, "\tbegin\n");
    fprintf(fp, "\t\ttest_runner_setup(runner, runner_cfg);\n");
    ...
    fprintf(fp, "\t\tcheck(RAM(19) = EXPECTED_OUTPUT, \"TEST FALLITO\");\n");
    fprintf(fp, "\t\ttest_runner_cleanup(runner);\n");
